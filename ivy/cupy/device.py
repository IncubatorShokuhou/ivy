...
def dev(x: cp.ndarray, as_native: bool = False) -> Union[ivy.Device, str]:
    if as_native:
        return "cpu"
...
def _to_device(x: cp.ndarray, device=None) -> cp.ndarray:
    """Private version of `to_device` to be used in backend implementations"""
...
def to_device(x: cp.ndarray, device: str) -> cp.ndarray:
    if device is not None:
