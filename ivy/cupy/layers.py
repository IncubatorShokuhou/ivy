"""Collection of Cupy network layers, wrapped to fit Ivy syntax and signature."""
...
def conv1d(
    ...
    x = cp.lib.stride_tricks.as_strided(x, shape=x_shape, strides=x_strides)
    filters = cp.lib.stride_tricks.as_strided(
        filters, shape=filter_shape, strides=filter_strides
    )
    res = conv2d(x, filters, strides, padding, data_format, dilations)
    res = cp.lib.stride_tricks.as_strided(
...
def conv1d_transpose(*_):
    raise Exception("Convolutions not yet implemented for cupy library")
...
def conv2d(
    ...
    sub_matrices = cp.lib.stride_tricks.as_strided(
        x, new_shape, new_strides, writeable=False
    )
    # B x OH x OW x KH x KW x I x O
    sub_matrices_w_output_dim = cp.tile(
        cp.expand_dims(sub_matrices, -1), [1] * 6 + [output_dim]
    )
    # B x OH x OW x KH x KW x I x O
    ...
    # B x OH x OW x O
    res = cp.sum(mult, (3, 4, 5))
    if data_format == "NCHW":
        return cp.transpose(res, (0, 3, 1, 2))
    return res
